...
freq_table = [400 + 100 * i for i in range(7)]
L = 47e-2
nb_phases = 36
demi_plage = 90

for f in freq_table:
    phasetab = [((360*(L/340)*f - 180)+((2*i/nb_phases)-1) * demi_plage) for i in range(nb_phases)]
    tab_rapport, _ = test_phasesf(phasetab, f)
    plt.plot(phasetab, tab_rapport, marker='x', ls='none')
    plt.xlabel("Phase en degré)")
    plt.ylabel("Amplitude sortie sur amplitude entrée")
    plt.title("Rapport amplitude sortie sur amplitude entrée en fonction du déphasage des sources")
    plt.savefig("Phase idéale pour f = " + str(f) + "Hz", format="png")
